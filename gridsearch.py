...
def grid_search(phase_list: list[Callable[[float], float]], params: dict) -> tuple:
    ...
    results = {tup: np.empty(shape=(len(phase_list),)) for tup in grid} # type: ignore
    
    for (i, phase) in enumerate(phase_list):
        print(f'Running parameter grid for phase profile {i+1}/{len(phase_list)}')
        ...
        results[(window_beta, padding, filter_width)][i] = score

    df = pd.DataFrame.from_dict(data=results, orient='index')
    df['Mean score'] = df.sum(axis=1)

    return df.index[df['Mean score'].argmin()] # type: ignore
